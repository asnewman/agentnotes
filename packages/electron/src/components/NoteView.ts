...
export class NoteView {
  ...
  private initEditor(): void {
    ...
    // Handle paste events (images and URLs)
    editorElement.addEventListener('paste', (event) => {
      ...
      // Check for URL pasted over selected text
      const text = event.clipboardData?.getData('text/plain');
      if (text && this.isUrl(text)) {
        const { from, to } = this.editor!.state.selection;
        if (from !== to) {
          // Text is selected - create a markdown link
          event.preventDefault();
          const selectedText = this.editor!.state.doc.textBetween(from, to);
          const markdownLink = `[${selectedText}](${text.trim()})`;
          this.editor!.chain().focus().deleteSelection().insertContent(markdownLink).run();
  ...
  private markdownLineToTipTap(line: string): string {
    ...
    if (imageMatch) {
      ...
      if (this.notesDirectory && !src.startsWith('http') && !src.startsWith('file://')) {
        src = `file://${this.notesDirectory}/${src}`;
      }
  ...
  private applyInlineMarkdownStyles(escapedLine: string): string {
    ...
    // Style markdown links with blue color
    withTokens = withTokens.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match) =>
      toToken(`<span style="color: #0078d4;">${match}</span>`),
    );
