...
export class MouseHandler {
    constructor(container, callbacks, positionCalculator, onFocus) {
        ...
        this.handleMouseDown = (event) => {
            ...
            this.onFocus();
            const offset = this.positionCalculator.getOffsetFromPoint(event.clientX, event.clientY, this.textElement);
            if (event.shiftKey) {
        ...
        this.handleMouseMove = (event) => {
            if (!this.isDragging || !this.textElement)
                return;
            event.preventDefault();
            const offset = this.positionCalculator.getOffsetFromPoint(event.clientX, event.clientY, this.textElement);
            this.callbacks.onSelectionChange?.({ anchor: this.dragAnchor, head: offset });
