...
const editor = new Editor(container, {
    onInsert: (position, text) => {
        ...
        // Adjust decorations
        state.decorations = adjustDecorationsForInsert(state.decorations, position, text.length);
        // Re-render
        editor.render(state);
        updateStateDisplay();
    },
 ...
 */
function adjustDecorationsForInsert(decorations, position, length) {
    return decorations
        .map((dec) => {
 ...
 */
function adjustDecorationsForDelete(decorations, from, to) {
    ...
    return decorations
        ...
        .filter((dec) => dec !== null && dec.from < dec.to);
}
/**
 ...
 */
function applyDecoration(type, attributes) {
    const { anchor, head } = state.selection;
    ...
    const existingIndex = state.decorations.findIndex((dec) => dec.from === from && dec.to === to && dec.type === type);
    if (existingIndex >= 0) {
        // Toggle off
